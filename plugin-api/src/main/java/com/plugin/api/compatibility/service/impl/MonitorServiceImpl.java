package com.plugin.api.compatibility.service.impl;
...
public class MonitorServiceImpl implements IMonitorService {
    ...
    private static final Log log = LogFactory.getLog(MonitorServiceImpl.class);
    
    // import
    // com.sun.management.OperatingSystemMXBean;//就在rt.jar那个包下。我这的是jdk6.0的，你的是不是5.0的
    ...
    private static double getCpuRateForLinux() {
        ...
        try {
            log.info("Get usage rate of CUP , linux version: "+ linuxVersion);
	...
	public static void createMiniPic(File file, float width, float height) throws IOException {
		...
		if (old_w >= width || old_h >= height) { // 如果文件小于锁略图的尺寸则复制即可
			...
			com.sun.image.codec.jpeg.JPEGEncodeParam param = encoder.getDefaultJPEGEncodeParam(tag);
